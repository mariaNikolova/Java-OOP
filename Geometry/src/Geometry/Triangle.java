...
public class Triangle extends PlaneShape{
	...
	public double getArea() {
		 ...
		 double area =Math.sqrt(semiPerimeter * (semiPerimeter - calcSideA()) * 
				 						(semiPerimeter - calcSideB()) * 
				 						(semiPerimeter - calcSideC()));
		 if(area <= 0 || String.valueOf(area).equals("NaN") ){
			 throw new IllegalArgumentException("The area can't be negative number");
		 }
		 else{
			 return area;
		 }
	...
	public String toString() {
		...
		String result = "";
		result += String.format("Triangle \n");
		for (int i = 0; i < newList.size(); i++) {
			result += String.format("%s%n", newList.get(i).toString2D());
		}
		result += String.format("Perimeter: " + Math.round(this.getPerimeter()) + " Area: " + Math.round(this.getArea()) + "\n");
		return result;
	}
