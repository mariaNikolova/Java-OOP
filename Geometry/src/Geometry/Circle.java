...
public class Circle extends PlaneShape {
	...
	public Circle(List<Vertice> newListOfVertices,double newRadius) {
	...
	public String toString() {
		...
		String result = "";
		result += String.format("Circle \n");
		for (int i = 0; i < newList.size(); i++) {
			result += String.format("%s%n", newList.get(i).toString2D());
		}
		result += String.format("Radius: %.2f",
				getRadius());
		result += String.format("\nPerimeter: " + Math.round(this.getPerimeter()) + " Area: " + Math.round(this.getArea()) + "\n");
		return result;
	}
